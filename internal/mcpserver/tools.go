...
func RegisterTools(s *server.MCPServer, movieService *MovieService) {
	...
	// 注册获取所有下载任务进度工具
	allDownloadProgressTool := mcp.NewTool(
		"all_download_progress",
	...
	// 注册删除下载任务工具
	removeDownloadTool := mcp.NewTool(
		"remove_download",
	...
	playableTodayMovieTvTool := mcp.NewTool(
		"playable_today_movie_tv",
	...
	// 注册最近24小时内更新为可播放状态的视频
	checkMovieIsPlayableTool := mcp.NewTool(
		"check_movie_is_playable",
	...
	// 注册检查电视剧某季某集是否可播放工具
	checkTVEpisodePlayableTool := mcp.NewTool(
		"check_tv_episode_is_playable",
