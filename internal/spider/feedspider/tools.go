...
func fetchMagnetDownLoad(baseFeeder BaseFeeder, selector string, videos []*types.FeedVideoBase) []*types.FeedVideoBase {
	...
	for _, video := range videos {
		...
		go func() {
			...
			if err != nil {
				log.WithCtx(context.Background()).Errorf("%s.%s %s http request url is %s, error:%s", baseFeeder.web, video.Type, video.TorrentName, video.TorrentURL, err)
				return
			}
			defer resp.Body.Close()
			doc, err := goquery.NewDocumentFromReader(resp.Body)
			if err != nil {
				log.WithCtx(context.Background()).Errorf("%s.%s %s http goquery error:%s", baseFeeder.web, video.Type, video.TorrentName, err)
				return
			}
